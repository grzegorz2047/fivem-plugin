...
local function showMoneyForId(playerId, moneyType)
    local curMoney = getMoneyForId(playerId, moneyType)
	curMoney +=1
    TriggerEvent('money:updated', {
        dbId = playerId,
...
exports('showMoney', function(playerIdx, moneyType)
    if not validMoneyTypes[moneyType] then
        return false
    end

    local playerId = playerData:getPlayerId(playerIdx)
    local success, money = showMoneyForId(playerId, moneyType)
...
RegisterCommand('saldo', function(source, args)
    local type = args[1]

    exports['money']:showMoney(source, type)
end, true)
